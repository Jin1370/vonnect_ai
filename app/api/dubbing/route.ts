...
async function getAudioDuration(filePath: string): Promise<number> {
    ...
    return new Promise((resolve) => {
        ...
        // We don't use -stats because it's for PROGRESS during encoding.
        exec(
            `"${ffmpegStatic}" -i "${filePath}"`,
...
// Check if the text ends with sentence-finishing punctuation
function endsWithSentenceFinisher(text: string): boolean {
    const trimmed = text.trim();
    // Don't split if it's just dots/punctuation with no letters/numbers
    if (!/[a-zA-Z0-9ㄱ-ㅎㅏ-ㅣ가-힣]/.test(trimmed)) return false;
    return /[.!?。！?]$/.test(trimmed);
}

// Merge word list into speech segments
// Split by: Speaker change, Sound effect token, OR Sentence ending punctuation
function buildSegments(words: ELWord[]): Segment[] {
    ...
    for (const w of words) {
        ...
        // Check if the current accumulated segment already ended with a sentence finisher
        const lastSegmentFinished = cur && endsWithSentenceFinisher(cur.text);

        // New segment starts if sound effect token, speaker changes, or previous segment ended a sentence
        if (
            !cur ||
            cur.speaker !== spk ||
            isSound ||
            isSoundWord(cur.text) ||
            lastSegmentFinished
        ) {
...
const INLINE_SOUND_PATTERN =
    /[\(\[\{《【「][^\)\]\}》】」]{1,40}[\)\]\}》】」]/g;
function cleanText(text: string): string {
    return text
        .replace(INLINE_SOUND_PATTERN, " ")
        .replace(/\s{2,}/g, " ")
        .trim();
}
...
async function extractAudioClip(
    ...
    return new Promise((resolve, reject) => {
        ffmpeg(sourcePath)
            .seek(start) // Slower but accurate (decodes up to fixed point)
            ...
            .on("error", (err: Error) =>
                reject(
                    new Error(
                        `Failed to extract original clip: ${err.message}`,
                    ),
                ),
            )
            .run();
...
async function mixAudioClips(
    ...
    try {
        ...
        wavBuf.write("RIFF", 0);
        wavBuf.writeUInt32LE(36 + dataBytes, 4);
        wavBuf.write("WAVE", 8);
        wavBuf.write("fmt ", 12);
        ...
        wavBuf.writeUInt32LE(dataBytes, 40);
        ...
        for (let i = 0; i < clips.length; i++) {
            ...
            const tempo = Math.min(
                2.0,
                Math.max(0.5, actualDuration / targetDuration),
            );
        ...
        filterParts.push(
            `${mixInputs}amix=inputs=${clips.length + 1}:normalize=0:dropout_transition=0[out]`,
        );
...
async function mergeAudioIntoVideo(
    ...
    return new Promise((resolve, reject) => {
        ffmpeg()
            ...
            .outputOptions([
                "-map",
                "0:v:0", // Original video track
                "-map",
                "1:a:0", // Dubbed audio track
                "-c:v",
                "copy", // Fast copy for video without re-encoding
                "-c:a",
                "aac", // Encode audio as AAC (MP4 compatible)
                "-shortest", // Exit based on shortest duration
                "-movflags",
                "+faststart", // Better for web/mobile streaming
            ])
            .output(outputPath)
            .on("end", () => resolve())
            .on("error", (err: Error) =>
...
async function cloneVoice(
    ...
    if (!res.ok)
        throw new Error(`Failed to create Voice Clone: ${await res.text()}`);
...
export async function POST(request: Request) {
    ...
    const USE_BASE_VOICE = process.env.USE_BASE_VOICE === "true";
    ...
    try {
        ...
        const targetLanguage =
            (formData.get("target_language") as string) || "en";
        ...
        sttForm.append(
            "file",
            new Blob([sttBuffer], { type: sttMime }),
            sttFileName,
        );
        ...
        if (words.length === 0)
            throw new Error("Could not extract speech text.");
...
Return a JSON object: {"segments": [{"speaker": "...", "translatedText": "..."}]}`,
        ...
        const sourceAudioForSample = isVideo
            ? extractedAudioPath
            : origVideoPath;
        ...
        for (const [speaker, segs] of Object.entries(speakerSegs)) {
            ...
            try {
                ...
                const sampleSecs = await buildSpeakerSample(
                    sourceAudioForSample,
                    segs,
                    samplePath,
                );
        ...
        // ────────────────────────────────────────────────────────────────
        // [STEP 3-B] Per-segment TTS (Using Clone or fallback voice) - Concurrency-limited parallel execution
        // ────────────────────────────────────────────────────────────────
        const MIN_CLIP_DURATION = 0.15;
        const CONCURRENCY_LIMIT = 2; // User's plan allows 3; set to 2 for safety
            ...
            const p = (async () => {
                ...
                if (isSoundSeg) {
                    ...
                    try {
                        ...
                        console.warn(
                            `Sound clip extraction failed for segment ${i}:`,
                            e,
                        );
                    }
                    results[i] = null;
                ...
                try {
                    ...
                    if (!ttsRes.ok)
                        throw new Error(
                            `TTS failed (${seg.speaker}): ${await ttsRes.text()}`,
                        );
        ...
        const translatedSummary = segments
            ...
            .filter(
                ({ s, t }) =>
                    s.text.trim().length > 0 && t && t.trim().length > 0,
            )
            .map(({ s, t }) => `[${s.speaker}] ${t}`)
            .join("\n");
