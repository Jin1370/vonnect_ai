...
-   **🎞️ 크롭(Crop) 범위 지정 기능**: 사용자 편의를 위해 자를 범위를 이중 슬라이더(Slider)로 직접 지정하고 미리보기로 확인할 수 있습니다.
-   **📜 번역 자막 표시 옵션**: 영상 내에 번역된 자막을 "Burn-in" 방식으로 입힐지 여부를 사용자가 직접 선택할 수 있습니다.
-   **▶️ 원본 영상 재생 기능**: 작업 전후로 시스템 내 플레이어를 통해 원본 영상을 바로 확인하며 더빙 결과와 비교할 수 있습니다.
-   **⚡ 클라이언트 기반 전처리**: FFmpeg WASM을 사용하여 대용량 파일도 브라우저에서 즉시 처리가 가능하도록 최적화했습니다.
...
USE_BASE_VOICE=false # true일 경우 클로닝을 생략하고 기본 목소리를 사용하여 할당량을 절약합니다.
```
